...
import { LayoutDashboard, Utensils, QrCode } from 'lucide-react';
...
const Navigation = () => {
  ...
  const navItems = [
    { path: '/recepcao', label: 'Recepcao', icon: <LayoutDashboard size={20} /> },
    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-slate-100 z-40 px-6 py-3 md:top-0 md:bottom-auto md:border-b md:border-t-0">
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        <div className="hidden md:flex items-center gap-2 font-bold text-blue-600 text-lg">
          <span className="text-2xl">Cafe</span> Hotel Breakfast Control
        </div>
              ...
              element={
                <div className="flex flex-col items-center justify-center h-full text-center space-y-6 pt-20">
                  ...
                  <p className="text-slate-500 max-w-md">
                    Escolha um dos acessos acima para iniciar o gerenciamento de cafe da manha.
                  </p>
                  <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
                    <Link
                      to="/recepcao"
                      className="p-4 bg-white rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all font-bold text-slate-700"
                    >
                      Recepcao
                    </Link>
                    <Link
