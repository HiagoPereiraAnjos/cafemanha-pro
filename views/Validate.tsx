...
import { ShieldCheck, User, MapPin, Building2, Calendar, QrCode, Camera, RefreshCw } from 'lucide-react';
...
const Validate: React.FC = () => {
  ...
  useEffect(() => {
    const validateToken = () => {
      ...
      const found = all.find(g => g.id === guestId);
      ...
      try {
        ...
        if (decoded.h.trim() !== found.room.trim()) {
  ...
  const confirmConsumption = () => {
    ...
    // Atualiza o hóspede
    storageService.updateGuest(guest.id, {
      usedToday: true,
      consumptionDate: today
    });
    
    // Dispara evento manual para abas locais (mesmo computador)
    ...
    <div className="max-w-md mx-auto space-y-6 pb-24">
      ...
      {!guestId && !isScanning && (
        <div className="bg-white p-10 rounded-3xl shadow-xl border border-slate-100 text-center space-y-6">
           ...
           <div className="space-y-2">
              ...
              <p className="text-slate-500">Use a câmera do celular ou o leitor integrado abaixo para validar um hóspede.</p>
           </div>
      ...
      {isScanning && (
        <div className="bg-white p-4 rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
           ...
           <Button variant="secondary" onClick={stopScanner} className="w-full mt-4">Cancelar Leitura</Button>
        </div>
      )}
      ...
      {error ? (
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-rose-100 text-center animate-in zoom-in">
           ...
           <Button className="w-full" onClick={handleReset}>Tentar Novamente</Button>
        </div>
      ) : guest && (
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-6">
          <div className={`p-6 text-center ${success ? 'bg-emerald-600' : 'bg-blue-600'} text-white transition-colors duration-500`}>
             <div className="text-4xl mb-2">{success ? '✅' : '☕'}</div>
             <h2 className="text-xl font-bold">{success ? 'Consumo Registrado!' : 'Dados do Hóspede'}</h2>
          </div>
            ...
            <div className="p-4 bg-slate-50 rounded-2xl space-y-2">
               <div className="flex justify-between items-center">
                  ...
                  <span className={`text-sm font-black ${guest.usedToday || success ? 'text-amber-600' : 'text-blue-600'}`}>
                    {success || guest.usedToday ? 'JÁ CONSUMIU' : 'DISPONÍVEL'}
                  </span>
               </div>
            </div>
            ...
            {!success ? (
               <div className="space-y-3">
                  <Button 
                    ...
                    disabled={guest.usedToday}
                  >
                    Confirmar Café da Manhã
                  </Button>
               ...
               <div className="text-center space-y-4">
                  <Alert type="success" message="Sincronizado! A recepção já recebeu a atualização em tempo real." />
